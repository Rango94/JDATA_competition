...
def multi():
    ...
    print(ca / len(Y_test))
    bst.save_model('../JDATA用户购买时间预测_A榜/xgboost_multi_local_test.model')
...
def reg():
    ...
    print(sum / len(Y_test))
    print(ca / len(Y_test))
    bst.save_model('../JDATA用户购买时间预测_A榜/xgboost_reg_local_test.model')
...
#二分类
    ...
    for i in range(len(Y_test)):
        if int(ylabel[i]+0.5) == Y_test[i]:
            sum+=1
    print(sum / len(Y_test))
    bst.save_model('../JDATA用户购买时间预测_A榜/xgboost_binary_local_test.model')
